...
module.exports = {
    create: async (req, res, next) => {
        try {
            ...
            for (let i = 0; i < body.participants.length; i++) {
                ...
                if (!valid) {
                    return res.notFound("Not Found", `Participan at index ${i} is invalid or deleted!`);
                }
            };
            const newCourse = await Course({
                ...
                participants: body.participants.map(participant => mongoose.Types.ObjectId(participant)),
            });
    ...
    search: async (req, res, next) => {
        try {
            ...
            let matchedOwners = [];
            if (query.owner) {
                matchedOwners = await User.find({ name: { $regex: query.owner, $options: 'i' }, deleted_flag: false }, '_id');
            }
